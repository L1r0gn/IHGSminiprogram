  ...
  filterHomeworks: function(filter) {
    ...
    switch(filter) {
      ...
      case 'PENDING':
        ...
        break;
      case 'SUBMITTED':
        filtered = this.data.homeworks.filter(hw => hw.status === 'SUBMITTED'||'WRONG_ANSWER'||'ACCEPT');
        break;
      case 'GRADED':
        filtered = this.data.homeworks.filter(hw => hw.status === 'GRADED'||'WRONG_ANSWER'||'ACCEPT');
        break;
      default:
        filtered = this.data.homeworks;
    }
...
doHomework: function(e) {
  ...
  wx.navigateTo({
    url: `/pages/myclass/doHomework/doHomework?id=${homeworkId}&redo=${isRedo}`
  });
},
